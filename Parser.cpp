...
static bool isIdentifier(const std::string& str) {
	if (str == keyword::Primary || str == keyword::Key || str == keyword::Table
			|| str == keyword::Create || str == keyword::Integer
			|| str == keyword::Numeric || str == keyword::Not
			|| str == keyword::on || str == keyword::Timestamp
			|| str == keyword::Null || str == keyword::Char
			|| str == keyword::Varchar)
		return false;
	return str.find_first_not_of("abcdefghijklmnopqrstuvwxyz_1234567890\"")
			== std::string::npos;
}
...
void Parser::nextToken(unsigned line, const std::string& token,
		Schema& schema) {
	if (getenv("DEBUG"))
	...
	if (token.empty())
		return;
	std::string tok;
	std::transform(token.begin(), token.end(), std::back_inserter(tok),
			tolower);
	switch (state) {
	...
	case State::Index: {
		if (isIdentifier(tok)) {
			...
			throw ParserError(line,
					"Expected IndexName, found '" + token + "'");
		}
	}
	case State::Table:
		if (isIdentifier(tok)) {
	...
	case State::IndexName: {
		if (tok == keyword::on) {
			state = State::ReadIndexOf;
		} else {
			throw ParserError(line, "Expected on, found '" + token + "'");
		}
		break;
	}
	case State::ReadIndexOf: {
		if (isIdentifier(tok)) {
			schema.indexes.back().ofTable = token;
			state = State::IndexOf;
		} else {
	...
	case State::CreateTableBegin:
		if (tok.size() == 1 && tok[0] == literal::ParenthesisRight) {
			...
			schema.relations.back().attributes.push_back(
					Schema::Relation::Attribute());
			schema.relations.back().attributes.back().name = token;
			state = State::AttributeName;
		} else {
			throw ParserError(line,
					"Expected attribute definition, primary key definition or ')', found '"
							+ token + "'");
		}
		break;
	case State::CreateIndexBegin:
		...
		//TODO: Test if else without condition is sufficient
		else {
			schema.indexes.back().attributeNames.push_back(tok);
		}
		break;
	...
	case State::Key:
		if (tok.size() == 1 && tok[0] == literal::ParenthesisLeft)
			...
			throw ParserError(line,
					"Expected list of key attributes, found '" + token + "'");
		break;
	case State::KeyListBegin:
		if (isIdentifier(tok)) {
			struct AttributeNamePredicate {
				const std::string& name;
				AttributeNamePredicate(const std::string& name) :
						name(name) {
				}
			...
			if (it == attributes.end())
				throw ParserError(line,
						"'" + token + "' is not an attribute of '"
								+ schema.relations.back().name + "'");
			schema.relations.back().primaryKey.push_back(
					std::distance(attributes.begin(), it));
			state = State::KeyName;
		} else {
			throw ParserError(line,
					"Expected key attribute, found '" + token + "'");
		}
		break;
	case State::KeyName:
	...
	case State::AttributeName:
		if (tok == keyword::Integer) {
			schema.relations.back().attributes.back().type =
					Types::Tag::Integer;
			...
			throw ParserError(line,
					"Expected type after attribute name, found '" + token
							+ "'");
		break;
	case State::AttributeTypeChar:
		if (tok.size() == 1 && tok[0] == literal::ParenthesisLeft)
			...
			throw ParserError(line,
					"Expected '(' after 'CHAR', found'" + token + "'");
		break;
	case State::CharBegin:
		if (isInt(tok)) {
			schema.relations.back().attributes.back().len = std::atoi(
					tok.c_str());
			state = State::CharValue;
		} else {
			throw ParserError(line,
					"Expected integer after 'CHAR(', found'" + token + "'");
		}
		break;
	case State::CharValue:
		if (tok.size() == 1 && tok[0] == literal::ParenthesisRight)
			...
			throw ParserError(line,
					"Expected ')' after length of CHAR, found'" + token + "'");
		break;
	case State::AttributeTypeVarchar: {
		if (tok.size() == 1 && tok[0] == literal::ParenthesisLeft)
			...
			throw ParserError(line,
					"Expected '(' after 'VARCHAR', found'" + token + "'");
		break;
	}
	case State::VarcharBegin:
		if (isInt(tok)) {
			...
			throw ParserError(line,
					"Expected integer after 'VARCHAR(', found'" + token + "'");
		}
		break;
	case State::VarcharValue:
		if (tok.size() == 1 && tok[0] == literal::ParenthesisRight)
			...
			throw ParserError(line,
					"Expected ')' after length of VARCHAR, found'" + token
							+ "'");
		break;
	case State::AttributeTypeNumeric:
		if (tok.size() == 1 && tok[0] == literal::ParenthesisLeft)
			...
			throw ParserError(line,
					"Expected '(' after 'NUMERIC', found'" + token + "'");
		break;
	case State::NumericBegin:
		if (isInt(tok)) {
			...
			throw ParserError(line,
					"Expected integer after 'NUMERIC(', found'" + token + "'");
		}
		break;
	case State::NumericValue1:
		if (tok.size() == 1 && tok[0] == literal::Comma)
			...
			throw ParserError(line,
					"Expected ',' after first length of NUMERIC, found'" + token
							+ "'");
		break;
	case State::NumericValue2:
		if (tok.size() == 1 && tok[0] == literal::ParenthesisRight)
			...
			throw ParserError(line,
					"Expected ')' after second length of NUMERIC, found'"
							+ token + "'");
		break;
	case State::NumericSeparator:
		if (isInt(tok)) {
			...
			throw ParserError(line,
					"Expected second length for NUMERIC type, found'" + token
							+ "'");
		}
		break;
	...
	case State::AttributeTypeTimestamp:
		if (tok.size() == 1 && tok[0] == literal::Comma)
			...
			throw ParserError(line,
					"Expected ',' or 'NOT NULL' after attribute type, found '"
							+ token + "'");
		break;
	case State::Not:
		if (tok == keyword::Null) {
			...
			throw ParserError(line,
					"Expected 'NULL' after 'NOT' name, found '" + token + "'");
		break;
	case State::Null:
		if (tok.size() == 1 && tok[0] == literal::Comma)
			state = State::Separator;
			...
			throw ParserError(line,
					"Expected ',' or ')' after attribute definition, found '"
							+ token + "'");
		break;
	default:
		throw;
